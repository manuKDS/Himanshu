import ActionBtn from "@/components/dashboard/genre/ActionBtn";
import InsertData from "@/components/dashboard/genre/InsertData";
...
const Genre = ({ genres }) => {
  ...
  dispatch(menuCurrent({ active_menu: "genre-list" }))
  ...
  useEffect(() => {
    ...
    let result = menus.find(item => item.menu_fid == 19)
  ...
  const tableReload = async () => {
    const res = await axiosInstance.get(process.env.API_SERVER + "genre");
  ...
  const searchHandler = () => {
    if (search == "") {
      ...
      const newList = genresList.filter((genre) =>
  ...
  const pageHandler = () => {
    const pageTotalVal = Math.ceil(genreList.length / pageEntries);
  ...
  useEffect(() => {
    const sorted = _.orderBy(genresList, ['genre'], [sortType]);
    setgenreList(sorted);
  }, [genresList, sortType]);
          ...
          <div className="relative overflow-x-auto mt-10 rounded-lg bg-gray-200 p-3">
            <table className="w-full border-separate border-spacing-y-2">
              ...
              <tbody className="text-xs">
                {genreList.slice(pageStart, pageEnd).map((data, index) => {
                  {
                    /* {genreList.map((data, index) => {  */
                  }
                  return (
                    <tr className="bg-white" key={data.genre_id}>
                      <td className="px-4 py-3 text-left ">{data.genre}</td>

                      <td className="px-4 py-3 text-left">
                        {data.is_excluded ? "No" : "Yes"}
                      </td>
                      <td className="px-4 py-3 text-left">
          ...
          {genreList.length > 0 &&
            <div className="flex gap-2 items-center justify-between pt-4 px-4">
              <span className="font-semibold text-sm">
                ...
                {genreList.length > pageEnd ? pageEnd : genreList.length} of{" "}
                {genreList.length} entries
              </span>
...
export async function getServerSideProps() {
  const { data, error } = await axiosInstance.get(process.env.API_SERVER + "genre");
  return {
    props: {
      genres: data,
    },
