import ActionBtn from "@/components/dashboard/Admin/TaxCreditApplication/ActionBtn";
import InsertData from "@/components/dashboard/Admin/TaxCreditApplication/InsertData";
...
const tabHeading = [
  "Tax Credit Application Type",
  "Application Sub- Type",
  "Province",
  "Tax Credit Application Name",
  "Last Modified",
  "Action",
];

export async function getServerSideProps() {
  const fetchAssistance = await axiosInstance.get(
    process.env.API_SERVER + "assistance"
  );
  ...
  return {
    props: {
      assistances: fetchAssistance.data,
...
const TaxCreditApplication = ({
  ...
  dispatch(menuCurrent({ active_menu: "tax-credit-application" }))
  ...
  useEffect(() => {
    ...
    let result = menus.find(item => item.menu_fid == 18)
  ...
  const tableReload = async () => {
    const res = await axiosInstance.get(process.env.API_SERVER + "assistance");
    if (res.status === 201) {
      setAssitanceList(res.data);
  ...
  const filteredResults =
    query === ""
      ? assitanceList
      : assitanceList.filter((filter) => {
        return filter.type.toLowerCase().includes(query.toLowerCase());
      });
  ...
  const pageHandler = () => {
    ...
    const pageTotalVal = Math.ceil(assitanceList.length / pageEntries);
  ...
  //----------------- sorting -----------------
  useEffect(() => {
    const sorted = _.orderBy(assistances, ['type'], [sortType]);
    setAssitanceList(sorted);
  }, [assistances, sortType]);
      ...
      <div className='p-5'>
        <div className='mx-auto'>
          ...
          <div className='px-3 py-5 rounded-lg border shadow bg-white'>
                    ...
                    .map((data, index) => {
                      return (
                        <tr className='bg-white' key={index}>
                          <td className='px-2 py-3 text-left'>{data.type}</td>
            ...
            {assitanceList.length > 0 &&
              <div className='flex gap-2 items-center justify-between pt-4 px-4'>
                <span className='font-semibold text-sm'>
                  Showing {pageStart + 1} to{" "}
                  {assitanceList.length > pageEnd
                    ? pageEnd
                    : assitanceList.length}{" "}
                  of {""}
                  {assitanceList.length} entries
                </span>
...
export default TaxCreditApplication;

TaxCreditApplication.getLayout = function getLayout(page) {
