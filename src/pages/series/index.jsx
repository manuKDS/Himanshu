import ActionBtn from "@/components/dashboard/series/ActionBtn";
import InsertData from "@/components/dashboard/series/InsertData";
...
const Series = ({ productions, distributorLst, genreLst, citysLst, provinceLst, countryLst, }) => {
  const dispatch = useDispatch()
  const search_visible = false;
  dispatch(changeVisible({ search_visible }))
  dispatch(menuCurrent({ active_menu: "series" }))
  ...
  const [ListPage, setListPage] = useState(productionList.filter(item => item.type === 2))
  ...
  useEffect(() => {
    ...
    let result = menus.find(item => item.menu_fid == 5)
            ...
            <table className="w-full border-separate border-spacing-y-2">
              ...
              <tbody className="text-xs">
                {ListPage.slice(pageStart, pageEnd).map((data, index) => {
                  if (data.parent_id === null && data.type == 2) {
                    return (
