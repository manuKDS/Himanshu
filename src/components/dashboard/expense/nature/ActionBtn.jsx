...
export default function ActionBtn({ data, state, fn }) {
  ...
  // const { expList, setExpList } = state;
  const { tableReload } = fn;

  // Get expense id from list
  const expId = data.expense_nature_id;

  async function deleteHandel(expenseId) {
    try {
      setLoading(true);

      const res = await axiosInstance.delete(
        process.env.API_SERVER + "expense_nature/" + expenseId
      );
      ...
      // setExpList(
      //   expList.filter((e) => {
      //     return e.expense_nature_id !== expenseId;
      //   })
      // );
