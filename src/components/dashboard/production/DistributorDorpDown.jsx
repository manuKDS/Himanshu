...
const DistributorDropdown = ({ list, convertOrgTypeName, state }) => {
  ...
  const filteredList =
    query === ""
      ? list
      : list.filter((org) => {
          return org.distributor.toLowerCase().includes(query.toLowerCase());
        });
  ...
  return (
    <Combobox
      value={distributor_fid}
      onChange={setdistributor_fid}
      as='div'
