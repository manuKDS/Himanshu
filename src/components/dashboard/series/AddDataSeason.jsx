...
export default function AddDataSeason({ isOpen, setIsOpen, state, productionId, tableReload }) {
  ...
  // foreign key fields
  const [status_fid, setstatus_fid] = useState(true);
  ...
  const insertData = async () => {
    ...
    if (production != "" && errorstatus == "" && province_fid != "" && incorporation_date != "") {    // && errorgenre =="" && errordistributor=="") {
      ...
      try {
        const res = await axiosInstance.post(process.env.API_SERVER + "production/season", data);
        setIsOpen(false);
        setorg_fid(1);
        setmodified_by(1);
        setproduction("");
              ...
              <div className='p-5'>
                <div className='space-y-3 text-sm'>
                  ...
                  <div className="grid grid-cols-1 gap-2 ">
                    <label  className="block mb-0 text-sm font-medium text-gray-700 dark:text-white ">Season Name</label>
