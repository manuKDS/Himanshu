...
export default function ActionBtn({ data, state, fn }) {
  ...
  const { prodBudgetList, setProdBudgetList } = state;
  const { tableReload } = fn;

  // Get expense id from list
  const budgetInfoId = data.tbl_production_budget_id;

  async function deleteHandel(budgetId) {
    try {
      setLoading(true);
      ...
      // setProdBudgetList(
      //   prodBudgetList.filter((e) => {
      //     return e.tbl_production_budget_id !== budgetId;
      //   })
      // );
      setLoading(false);
    } catch (error) {
