...
const ProvinceDropdown = ({ list, convertProvinceId, state, error = false }) => {
  ...
  const filteredList =
    query === ""
      ? list
      : list.filter((item) => {
        return item.province.toLowerCase().includes(query.toLowerCase());
      });
    ...
    >
      <div className='relative'>
        <Combobox.Input
          ...
          className={error ? 'px-5 h-[45px] w-full bg-red-200 rounded-md outline-none placeholder:text-gray-500' : 'px-5 h-[45px] w-full bg-[#E9E9E9] rounded-md outline-none placeholder:text-gray-500'}
        // className={
        //   errorcity != ""
