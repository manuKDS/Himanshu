...
export default function ActionBtn({ data, state, fn }) {
  ...
  // const { assitanceList, setAssitanceList, provinceList } = state;

  const { tableReload } = fn;

  // Get assistance id from list
  const assistance_id = data.tax_assitance_id;

  async function deleteHandel(id) {
    try {
      setLoading(true);
      const res = await axiosInstance.delete(
        process.env.API_SERVER + "assistance/" + id
      );

      if (res.status !== 200) throw error;

      // setAssitanceList(
      //   assitanceList.filter((e) => {
      //     return e.tax_assitance_id !== assistance_id;
      //   })
      // );
