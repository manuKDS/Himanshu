...
export default function ActionBtn({ data, state, fn, parentCategories }) {
  ...
  async function deleteHandel(id) {
    try {
      ...
      const res = await axiosInstance.delete(process.env.API_SERVER + "expense_categories/" + id);
  ...
  return (
    <div>
      ...
      <DeleteDailog
        fn={{ deleteHandel }}
        state={{ isDelOpen, handleCloseDelete, loading, isDelError }}
        data={{ categoryId }}
      />
    </div>
  );
