...
export default function DeleteDailog({ state, fn, data }) {
  const { isDelOpen, handleCloseDelete, loading, isDelError } = state;
          ...
          >
            <Dialog.Panel className='mx-auto w-[400px] rounded-[15px] bg-white overflow-hidden'>
              ...
              <div className='px-5 pt-5 pb-6'>
                {isDelError && <div className='space-y-3 mb-5 text-red-500'>
                  The expence title in this category needs to be removed first.</div>}
                <div className='flex gap-x-4 justify-end'>
                  <button
                    className='px-6 py-2.5 min-w-[140px] bg-red-400 text-white font-semibold rounded-md'
                    onClick={() => deleteHandel(categoryId)}
