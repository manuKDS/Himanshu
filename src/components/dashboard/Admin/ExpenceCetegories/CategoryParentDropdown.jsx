...
const CategoryParentDropdown = ({ list, state, error = false }) => {
  const { categoryParent, setCategoryParent } = state;
  ...
  const filteredList =
    query === ""
      ? list
      : list.filter((item) => {
        return item.value.toLowerCase().includes(query.toLowerCase()) || item.description.toLowerCase().includes(query.toLowerCase());
      });
        ...
        <Combobox.Options className='absolute mt-1 max-h-[80px] overflow-y-auto w-full rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-gray-300 ring-opacity-5 focus:outline-none sm:text-sm z-10'>
          {filteredList.map((active, index) => (
            <Combobox.Option
              ...
              value={active.value}
            >
              {active.value}: {active.description}
            </Combobox.Option>
          ))}
        </Combobox.Options>
      )}
